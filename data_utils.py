...
class MovieReviewsProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir,is_boostRNP='no'):
        if is_boostRNP == 'no':
            return self._create_examples(
            self._read_data_with_block(os.path.join(data_dir, "train.txt"),'train'))
        else:
            return self._create_examples(
            self._read_data_with_block_is_boostRNP(os.path.join(data_dir, "train.txt"),'train'))

    def get_dev_examples(self, data_dir):
        return self._create_examples(
            self._read_data_with_block(os.path.join(data_dir, "dev.txt"),'dev'))

    def get_test_examples(self, data_dir):
        return self._create_examples(
            self._read_data_with_block(os.path.join(data_dir, "test.txt"),'test'))
    ...
    def _read_data_with_block(self, filename,data_type):
        ...
        if data_type == 'train':
            content_lines = open(filename).readlines()#[0:10]
            tag_lines = open(filename + ".block").readlines()#[0:10]
    ...
    def _read_data_with_block_is_boostRNP(self, filename,data_type):
        print('_read_data_with_block_is_boostRNP')
        if data_type == 'train':
            content_lines = open(filename).readlines()#[0:100]
            tag_lines = open(filename + ".block").readlines()#[0:100]
        else:
            content_lines = open(filename).readlines()#[0:10]
            tag_lines = open(filename + ".block").readlines()#[0:10]
            ...
            for idx in range(len(tag_lines)):
                ...
                if random.random() > self.fraction_rationales:
                    ...
                    tag_lines[idx] = " ".join(["-1" for _ in \
                        range(len(tag_lines[idx].strip().split()))])
        xx = []
        for index,data in enumerate(content_lines):
            xx.append([data,tag_lines[index]])
        random.seed(1)
        random.shuffle(xx)
        content_lines1 = [ i[0] for i in xx]
        tag_lines1 = [ i[1] for i in xx]
        return [*zip(content_lines1, tag_lines1)]
...
class DatasetWithRationales(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.dataset in ['movie_reviews', 'esnli', 'evinf', 'multirc','fever','boolq','sst2','IMDB']:  # modified add
